...
{
    //prevent sending form without some basic checks
    $('#usermgmt-form-import-data-send').on('click', function(event) {
	...
	{	
		...
		if ( (value[0] < 8) ) //prevent button to delete default groups
        { 
           value.push('');
        }
        else
...
function addFormtoDialog(usernameTitle)
{
	...
	$( '#expire, #lastUpdate, #created' ).datepicker({ 
		changeYear: true,
	});
		...
		success:function(data, textStatus, jqXHR)
				{
					...
					$.each( data.userdata, function( key, value ) {
						...
						else if (key == 'EULA_accepted')
						{
							if( value != 0)
							{
								$('#EULA_accepted').attr('checked', 'checked');
							}
						}
						else
						{
...
/* This function handle file upload to server to mass import users 
 * from csv data
 */
function startImport()
{
    //check user fields
    if ( $("#expire-days").val() == false || $("#coord-system").val() == false ){
        showDialog(L_errors_ERROR, L_general_complete_field);
        return false;
    }
    
    //check file field
     if ( document.getElementById("usermgmt-import-file").files.length == 0 ){
        showDialog(L_errors_ERROR, L_usermgmt_select_file);
        return false;
    }
    ...
    form_data.append('coord-system', $("#coord-system").val() );
    form_data.append('action', 'import');
    
    //preparing files to send
    var fileInput = $('#usermgmt-import-file').get(0); 
