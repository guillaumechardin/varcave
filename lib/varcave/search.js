...
function searchSuccess (json,state)
{
	...
	trContent += '<th class="searchStatFooter" colspan="'+numCOl+'" > ' + search_totalDepth + ' : ' + json.totalDepth + 'm | ' + search_totalLength + ': ' + json.totalLength + 'm';
	
	downloadGPXbutton = downloadGPXbutton.replace('{json.searchid}', json.searchid);
	
	trContent +=  downloadGPXbutton +  '</th>';	
	trContent += '</tr>';
...
/* start gpx download from current search*/
$(document).on('click', '#search-gpx-download', function(e)
{
	var searchid = $(this).data('searchid');
	var url='getgpxkml.php?action=searchgpxdownload&searchid=' + searchid;
	window.open(url);
	return true;
});