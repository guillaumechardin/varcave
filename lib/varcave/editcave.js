$(document).ready(function ()
{
    ...
    $('body').on('click', 'span[id|="edit-delCoordSet"]',  function() {
		var coordsSetIdx = $(this).parent('.editCoords').attr('data-coordset');
        console.log('try to delete: ' + $(this).attr('id') );
		console.log('array id:'+coordsSetIdx);
		jqEl = $(this);
		console.log('html element ' + jqEl);
        updateCaveCoords(coordsSetIdx,'', '', jqEl, add = false, del = true, modify = false)
	});
    ...
    $('body').on('change', 'input[class="coords"]', function(){
        ...
        if(value == null || $.trim(value) == '' ){
            console.log('Currently empty field');
            return true;
        }
        
        //disable default behavior if not geographic, first reprocess coords to convert it as geo
        if( coordSystem != 'GEOGRAPHIC'){
            console.log('convert coords before sending data to server');
            
            //get converted coordinates for current selected coordsystem
            var newCoords = window[coordSystem+'_2longlat'](coordSetIdx, valueIdx);
            
            console.log('cave guid: ' + guid);
            console.log('Changing coords set index: ' + coordSetIdx);
            console.log('coord to edit: X=0/Y=1/Z=2 :'+valueIdx)
            console.log('with new value: ' + newCoords[valueIdx] );
            
            updateCaveCoords(coordSetIdx, 0, newCoords[0], jqEl,false ,false, true);
            updateCaveCoords(coordSetIdx, 1, newCoords[1], jqEl,false ,false, true);
            updateCaveCoords(coordSetIdx, 2, newCoords[2], jqEl,false ,false, true);
            
            //update global var to reflect changes when going back to geographic coordinates
            coordinatesList.features[0].geometry.coordinates[coordSetIdx] = [ newCoords[0], newCoords[1], newCoords[2] ];
            console.log('new coordinatesList data : ');
		...
		console.log('coord idx X/Y/Z:'+valueIdx)
		console.log('with new value: ' + value);
		...
		success: function(data, textStatus, jqXHR)
				{
					...
					if(data.actionType == 'delete')
					{
                        ...
                        (coordinatesList.features[0].geometry.coordinates).splice(coordSetIdx, 1);
                        //hide and delete from DOM element
						$(jqEl).parent().hide('slow', function ()
							{
								$(this).remove();
						...
						var el = $('.editCoords');
						el.each(function (){
							var currentElNumber = $(this).attr('data-coordset');
							if( currentElNumber > coordSetIdx )
							{
								var newVal = $(this).attr('data-coordset') - 1;
								console.log('renumber element['+currentElNumber+'] to: ['+newVal+']');
								$(this).attr('data-coordset', newVal);
							}
						});
                    ...
                    if(data.actionType == 'add')
                    {
                        //set default value of 1 to prevent conversion error if user change coordsystem
						var newHtml =''; 
						newHtml += '<div class="editCoords" data-coordset="' + data.insertIndex + '">'; 
                        newHtml += '   X:<input type="text" class="coords" data-elNumber="0" value="1" />';
                        newHtml += '   Y:<input type="text" class="coords" data-elNumber="1" value="2" />';
                        newHtml += '   Z:<input type="text" class="coords" data-elNumber="2" value="3" />';
                        newHtml += '  &nbsp<span class="fas fa-trash-alt fa-lg"  id="edit-delCoordSet" ></span>';
                        
                        (coordinatesList.features[0].geometry.coordinates).push( [1,2,3] );
                        console.log(coordinatesList);
                    ...
                    if(data.actionType == 'modify'){
                     //delete data from coordinatesList var 
                    coordinatesList.features[0].geometry.coordinates[coordSetIdx][valueIdx] = parseInt(value, 10);
                    }
