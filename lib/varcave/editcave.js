		...
		success: function(data, textStatus, jqXHR)
				{
					...
					if(data.actionType == 'del')
					{
                        ...
                        console.log('array avant modif');
                        console.log(JSON.parse(JSON.stringify(coordList) ) );
                        
                        
                        var longit = jqEl.parent('.editCoords').children('input[data-coord="long"]').val();
                        var latitude = jqEl.parent('.editCoords').children('input[data-coord="lat"]').val();
                                           
                       coordList.features[0].geometry.coordinates.forEach( function(item,i) {
                            console.log(item+' :: search'+longit+'|'+latitude);
                            //var return2  = item.indexOf(data);
                            if( item.indexOf(longit) != -1 && item.indexOf(latitude) != -1){
                              console.log('array found'+i+'::..');
                              coordList.features[0].geometry.coordinates.splice(i,1);
                    ...
                    if(data.actionType == 'add')
                    {
                        ...
                        //add data to coordList
                        coordList.features[0].geometry.coordinates.push([values.long, values.lat , values.z]);
